...
class DAEDisaggregatorFE(Disaggregator):
    ...
    def train_on_chunk(self, mainchunk, meterchunk, epochs, batch_size):
        ...
        X_batch = np.reshape(X_batch, (int(X_batch.shape[0] / s), s, X_batch.shape[1]))      
...
#        model.add(Conv1D(8, 4, activation="linear", input_shape=(sequence_len, 1), padding="same", strides=1))
        model.add(Conv1D(8, 4, activation="linear", input_shape=(sequence_len, 28), padding="same", strides=1))
        model.add(Flatten())
