# Source code was originally from the following site
#  - https://github.com/OdysseasKr/neural-disaggregator
# We do not modify original source codes except for adding feature extracting method, "_feature_extract".
# Some parts calling the method were also changed. Also, parameter setting for the changed input data was also affected.
# Please refer to the source commentation
...
class ShortSeq2PointDisaggregatorFE(Disaggregator):
    ...
    def train_on_chunk(self, mainchunk, meterchunk, epochs, batch_size):
        ...
        mainchunk = np.reshape(mainchunk, (int(mainchunk.shape[0]/self.window_size),self.window_size,mainchunk.shape[1]))
    ...
    def disaggregate_chunk(self, mains):
        ...
        print('feature selection is done..')       
        X_batch = np.reshape(X_batch, (int(X_batch.shape[0]/self.window_size),self.window_size,X_batch.shape[1]))
    ...
    def _feature_extract(self, active):
    ...
    def _create_model(self):
        ...
        model.add(Conv1D(30, 10, activation='relu', input_shape=(self.window_size,28), padding="same", strides=1))
